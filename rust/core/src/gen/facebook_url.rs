...
use super::helpers::handle::{
    gen_handle, unique_tag, HandleArchetype, HandleInput, FACEBOOK_POLICY, FACEBOOK_SEPS,
};
...
pub fn gen(ctx: &mut GenContext<'_>, buf: &mut String) {
    ...
    let tag = unique_tag(ctx.rng.record(), 0xFB01);
    buf.push_str("https://facebook.com/");
    let h = HandleInput {
        first,
        last,
        tag,
